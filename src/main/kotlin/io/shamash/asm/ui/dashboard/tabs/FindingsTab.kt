 ...
 *
 * Aggregates:
 * - PSI findings (lightweight deterministic scan using the same rule helpers we already ship)
 * - ASM findings (pure function from AsmIndex)
 *
 * Filtering:
 * - module
 * - package prefix
 * - inferred layer (cheap string inference)
 */
class AsmFindingsTabPanel(private val project: Project) : JPanel(BorderLayout()) {
    ...
    private val packageField = JBTextField().apply {
    ...
    init {
        val top = JPanel(BorderLayout()).apply {
            val filters = JPanel(FlowLayout(FlowLayout.LEFT, 8, 6)).apply {
                add(JBLabel("Sources"))
                add(includeAsm)
                add(includePsi)
        ...
        packageField.document.addDocumentListener(object : com.intellij.ui.DocumentAdapter() {
    ...
    fun onIndexUpdated(newIndex: AsmIndex) {
        ...
        // refresh module list from ASM immediately; PSI will add more later
        refreshModuleBox(buildModuleList(asmFindings, psiFindings))

        header.text = "Findings: ASM=${asmFindings.size}. PSI: computing…"
        applyFiltersAndRender()

        // PSI scan in background (deterministic read action)
        kickOffPsiScan()
    }

    private fun kickOffPsiScan() {
        ReadAction.nonBlocking<List<Finding>> {
    ...
    private fun applyFiltersAndRender() {
        ...
        val includeSources = mutableSetOf<Source>().apply {
    ...
    private fun buildModuleList(a: List<Finding>, b: List<Finding>): List<String> {
        return (a.asSequence().mapNotNull { it.module } + b.asSequence().mapNotNull { it.module })
            .map { it.trim() }
    ...
    // ---------------------- Engines (kept in-file on purpose: Shamash way) ----------------------
    ...
    private object AsmFindingsEngine {
        ...
        fun fromIndex(index: AsmIndex): List<Finding> {
            ...
            fun severityForGod(score: Int): Severity = when {
            ...
            fun severityForDepth(depth: Int): Severity = when {
                depth >= 10 -> Severity.HIGH
                depth >= 7 -> Severity.MEDIUM
                else -> Severity.LOW
            }

            fun severityForFanOut(fanOut: Int): Severity = when {
    ...
    private object PsiFindingsEngine {
        ...
        fun scanProject(project: Project): List<Finding> {
            ...
            for (psiClass in query) {
                ...
                val moduleName = com.intellij.openapi.module.ModuleUtilCore.findModuleForPsiElement(psiClass)?.name
                ...
                // Unused methods: keep deterministic + bounded
                val methods = psiClass.methods.asSequence()
                    .filter { it.name != "<init>" }
                    .take(200)
                    .toList()
