 ...
 * - fan-out(node) = number of distinct outgoing edges.
 * - If any node fan-out > max -> emit a single finding listing top violators (deterministic, truncated).
 */
class MaxFanOutRule : Rule {
    override val id: String = "metrics.maxFanOut"
    ...
    override fun evaluate(
        ...
        return listOf(
            Finding(
                ...
                message =
                    "Fan-out violations detected: ${violators.size} " +
                        "node(s) exceed max (${params.max}) at granularity '" +
                        "${params.granularity.name.lowercase()}'.",
